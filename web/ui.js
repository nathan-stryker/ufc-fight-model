(function () {
  ...
  // Bout-card preview: the likely winner with their win %, then how the fight
  // is likely to end as percentages rather than one "by Decision" pick --
  // Decision is the single most common outcome in most matchups, so a
  // one-word pick hid most of what the method model actually says. Fixed
  // Decision/KO/Sub order so the numbers line up card to card; the most
  // likely one is emphasized. No round here: the round model is plain base
  // rates by fight length (see train_round.py), so it stays in the Breakdown.
  const METHOD_SHORT = { dec: "Decision", ko: "KO/TKO", sub: "Sub" };
  function modelPickHtml(r) {
    const v = verdictText(r);
    const winPct = Math.round((v.aWinner ? r.probAWins : r.probBWins) * 100);
    const chips = ["dec", "ko", "sub"].map((m) =>
      `<span class="fc-method-chip${m === v.topMethod ? " is-top" : ""}">${METHOD_SHORT[m]} ${Math.round(r.method[m] * 100)}%</span>`
    ).join("");
    return `<div class="fc-model-pick mono">` +
      `<span class="fc-model-pick-label">Model predicts</span> ${escapeHtml(v.winnerName)} ` +
      `<span class="fc-model-pick-pct">${winPct}%</span>` +
      `<div class="fc-method-split">${chips}</div></div>`;
  }
  ...
  function renderUpcomingCard() {
    ...
    function boutRowHtml(b) {
      ...
      if (predictable) {
        ...
        modelPick = modelPickHtml(result);
